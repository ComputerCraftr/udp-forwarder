...
fn stress_ipv4_one_minute() {
    ...
    let listen = wait_for_listen_addr(&mut child, std::time::Duration::from_secs(2));

    // Load gen for 60 seconds
    let client = bind_udp_v4_client();
    let payload = vec![0u8; 200];
    client.send_to(&payload, listen).unwrap(); // lock to this client
    ...
    let recv_thr = std::thread::spawn(move || {
        let mut rcvd = 0u64;
    ...
    let stats = wait_for_stats_json(&mut child, std::time::Duration::from_secs(2));
